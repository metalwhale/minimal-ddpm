...
from .data import MultimodalDistribution, plot_histogram
...
def train(
    ...
    target_distribution: MultimodalDistribution,  # Target distribution
    model: MinimalDdpm,
    ...
    with open(storage_path / "log.csv", "w") as log_file:
        ...
        for epoch in range(epoch_num):
            ...
            if epoch % 100 == 0:
                ...
                log_file.flush()
                _eval(storage_path, target_distribution, model, s, dt, f"{epoch:0{len(str(epoch_num))}d}")
    return model


def _eval(
    storage_path: os.PathLike,
    target_distribution: MultimodalDistribution,
    model: MinimalDdpm,
    s: float,
    dt: float,
    image_name: str,
    ...
    model_samples = _inference(model, s, dt, sample_num)
    # Draw the histograms
    ...
    Image.fromarray(np.hstack([target_image, model_image])).save(storage_path / f"{image_name}.png")
...
def _inference(
    ...
    xt_batch = []
    for _ in range(batch_size):
        xt_batch.append(np.random.normal(scale=s))
    with torch.no_grad():
        ...
        for t in np.arange(1.0, 0.0, -dt):
            ...
            x_batch = torch.cat((xt_batch, t_batch), dim=1)
            xt_batch = model(x_batch)
            n_batch = [np.random.normal(scale=s * math.sqrt(dt)) for _ in range(batch_size)]  # Gaussian noise
